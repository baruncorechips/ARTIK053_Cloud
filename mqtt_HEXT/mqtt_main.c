...
#define DEVICE_ID "da11183038f04b5d9ec3c59c6c713cea" //FIX
#define DEVICE_TOKEN "c95fcae523d3451497f51ccea9181ab0" // FIX
#define ACTION_TOPIC "/v1.1/actions/da11183038f04b5d9ec3c59c6c713cea" // FIX
#define MESSAGE_TOPIC "/v1.1/messages/da11183038f04b5d9ec3c59c6c713cea" //FIX
...
void onMessage(void* client, mqtt_msg_t *msg)
{
	...
	if (strstr(buf,"GPIO46ON") != NULL) gpio_write(PIN_D2,HIGH);
	else if (strstr(buf,"GPIO46OFF") != NULL) gpio_write(PIN_D2,LOW);

	if (strstr(buf,"GPIO47ON") != NULL) gpio_write(PIN_D4,HIGH);
	else if (strstr(buf,"GPIO47OFF") != NULL) gpio_write(PIN_D4,LOW);

	if (strstr(buf,"GPIO48ON") != NULL) gpio_write(PIN_D7,HIGH);
	else if (strstr(buf,"GPIO48OFF") != NULL) gpio_write(PIN_D7,LOW);

	if (strstr(buf,"GPIO50ON") != NULL) gpio_write(PIN_D8,HIGH);
	else if (strstr(buf,"GPIO50OFF") != NULL) gpio_write(PIN_D8,LOW);


	else if(strstr(buf,"pwm") != NULL)// Action is related to PWM
	{
		//Extract pwm port
		int ref = 0;
		char tempBuf[2];
		char* p = strstr(buf,"pwm");
		while(*p)
		{
			if (isdigit(*p))
			{
		...
		freqBuf[ref] = '\0';
		int freqValue = atoi(freqBuf);
		printf("Frequency values is %d\n",freqValue);
		...
		while(*p)
		{
			if (isdigit(*p)) dutyBuf[ref++] = *p;
			p++;
		}
		dutyBuf[ref] = '\0';
		int dutyValue = atoi(dutyBuf);
		printf("Duty value is %d\n",dutyValue);

		int fd = pwm_open(port);
		pwm_write(fd,freqValue,dutyValue);
		pwm_close(fd);
	}
}
...
int main(int argc, char *argv[])
{
	...
	while (mqttSubscribe == false )
	{
		...
		while (1)
		{
			...
			char tempBuf[20];
			sprintf(tempBuf,"\"Channel0\":%d", read_adc(0)); strcat(str,tempBuf); strcat(str,",");
			sprintf(tempBuf,"\"Channel1\":%d", read_adc(1)); strcat(str,tempBuf); strcat(str,",");
			sprintf(tempBuf,"\"Channel2\":%d", read_adc(2)); strcat(str,tempBuf); strcat(str,",");
			sprintf(tempBuf,"\"Channel3\":%d", read_adc(3)); strcat(str,tempBuf); strcat(str,",");
			strcat(str,"},");
			...
			strcat(str,"\"GPIO\":{");
			char buf[10];
			sprintf(buf,"\"gpio%d\":%d",46,gpio_read(46)); strcat(str,buf); strcat(str,",");
			sprintf(buf,"\"gpio%d\":%d",47,gpio_read(47)); strcat(str,buf); strcat(str,",");
			sprintf(buf,"\"gpio%d\":%d",48,gpio_read(48)); strcat(str,buf); strcat(str,",");
			sprintf(buf,"\"gpio%d\":%d",50,gpio_read(50)); strcat(str,buf);
			...
			else
			{
				...
				char buff[100];
				strcpy(buff,str);
				char *p = strstr(buff,"\"GPIO");
				printf( "%s\n",p);
				char bufff[100];
				int i;

				for(i = 0; i<(p-buff-1);i++)
				{
					bufff[i]=*(buff+i+1);
				}
				bufff[p-buff-2]='\0';
